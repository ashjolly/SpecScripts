...
p_load(lubridate,plyr,ggplot2,reshape2,car,grid,gridBase,gridExtra, taRifx,magrittr, ggpmisc,
       gplots, plyr, d3heatmap, RColorBrewer, scales, nlme, viridis)
...
negpos_palette <- colorRampPalette(c("#E69F00", "light grey", "#56B4E9"))(n = 299)

# colour pallet for going 0 - 100 (percent)
per_palette <- colorRampPalette(c("light grey", "#0072B2"))(n = 299)

# colou rpallet for non=ercentage
ind_palette <- colorRampPalette(c("light grey", "#E69F00"))(n = 299)
...
# get rid of periods in the stakeholder names
names(data) <- gsub(x = names(data),
                        pattern = "\\.",
                        replacement = " ") 
# Sub in shorter names for sub policy areas


# Get rid of mining?

########### bar graph - number of respondents in stakeholder group
# Bar graph of the number of respondents by stakeholder group
sum.total <- subset(data, data$Status == "Submitter Category")
colnames(sum.total)[5] <- "Individuals - form"
colnames(sum.total)[6] <- "Individuals - nonform"

# change order of columns
sum.total.order <-sum.total[,c(5:21, 26)][,order(colnames(sum.total[,c(5:21, 26)]),decreasing=FALSE)]
...
plot <- barplot(as.matrix(sum.total.order), main = "", cex.main = 2, 
                cex.axis = 2,
                col = "#56B4E9", axes = FALSE, axisnames = FALSE)
par(op)        
text(plot, par("usr")[3]-.5, labels = (colnames(sum.total.order)),srt = 45,
...
png(paste(directory, "WSA_number of submissions_noind.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
heatmap.2(mat.data,
          ...
          trace="none",         # turns off trace lines inside the heat map
          margins =c(8,20),     # widens margins around plot
          col=negpos_palette,       # use on color palette defined earlier 
          cexCol=1, 
          cexRow = 0.6,          # decrease row font size to fit
          #breaks=col_breaks,    # enable color transition at specified limits
          dendrogram="row",     # only draw a row dendrogram
...
png(paste(directory, "WSA_response_subpolicy_nonumb.png", sep = ""),    # create PNG for the heat map        
    width = 11*300,        # 5 x 300 pixels
    height = 11*300,
    res = 600,            # 300 pixels per inch
    pointsize = 5)        # smaller font size
...
for (i in 1:n){
    temp.influence <- data.frame(t(apply(response.all[i,2:17], 2, function(x) x - response.all[i,20])))
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_InfluenceMap_columngrouping_nonum.png", sep = ""),    # create PNG for the heat map        
    width = 10*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_groundwater_nonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_allocation_nonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_EFN_nonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_ERGA_nonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
#grid.draw(editGrob(a, vp=viewport(a,layout.pos.row = 1, 
#                                  layout.pos.col = 1, clip=TRUE)))
#grid.draw(editGrob(b, vp=viewport(b,layout.pos.row = 1, 
...
# combine 'health' with community groups (only one respondent in 'health')
data.original$Community.Groups <- data.original$Community.Groups + data.original$Health
...
# get rid of periods in the stakeholder names
names(sub.all) <- gsub(x = names(sub.all), pattern = "\\.",replacement = " ") 
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_PercentRespondents_subpolicy.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# creates a 5 x 5 inch image - without numbers
png(paste(directory, "WSA_PercentRespondents_subpolicynonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# get rid of periods in the stakeholder names
names(main.all) <- gsub(x = names(main.all), pattern = "\\.",replacement = " ") 
...
heatmap.2(mat.data,
          ...
          Colv=TRUE)            # turn off column clustering
dev.off()
# No numbers in heat map
# creates a 5 x 5 inch image
png(paste(directory, "WSA_PercentRespondents_main_nonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size
...
# creates a 5 x 5 inch image
png(paste(directory, "WSA_Comments_heatmap_per_nonum.png", sep = ""),         # create PNG for the heat map        
    width = 20*300,        # 5 x 300 pixels
    height = 10*300,
    res = 600,            # 300 pixels per inch
    pointsize = 6)        # smaller font size#
...
heatmap.2(mat.data,
          ...
          # appearance
          margins =c(20,20),     # widens margins around plot
          col= ind_palette,       # use on color palette defined earlier 
          cexCol=1.2, 
          cexRow = 1.5,          # decrease row font size to fit
          srtCol=45,           # rotate the x labels at 45 deg so they fit
          #axisnames = FALSE,
