...
EEMcorrection = function(data.3, directoryall, directoryCorrectedEEMS){
  ...
  # Create blank dataframe for Raman corrections
  Raman <- data.frame(0,0)
  ...
  for (i in 1:600){
    ...
    # DO the same for the blank file
    Blank.interp <- as.data.frame(apply(Blktrim, 2, function(g) approx(x = as.numeric(rownames(Blktrim)), y = g, xout = em.seq)$y))
    row.names(Blank.interp) <- em.seq 
    
    # if the excitation doesn't go in 2 nm increments, interpolate so that it does 
    if (colnames(EEM.interp)[1] - colnames(EEM.interp)[2] != 2){
      ex.seq <- seq(as.numeric(format(round(as.numeric(min(colnames(EEM.interp))), 0), nsmall = 0)), as.numeric(format(round(as.numeric(max(colnames(EEM.interp))), 0), nsmall = 0)), by = 2)
      EEM.interp2 <- as.data.frame(apply(EEM.interp, 1, function(g) approx(x = as.numeric(colnames(EEM.interp)), y = g, xout = ex.seq)$y))
      colnames(EEM.interp) <- ex.seq
      row.names(EEM.interp) <- em.seq
      
      # DO the same for the blank file
      Blank.interp <- as.data.frame(apply(Blank.interp, 1, function(g) approx(x = as.numeric(colnames(Blank.interp)), y = g, xout = ex.seq)$y))
      colnames(Blank.interp) <- ex.seq 
    ...
    em375 <- as.numeric(grep("375", rownames(Blank.interp)))
    em430 <- as.numeric(grep("430", rownames(Blank.interp)))
    ex350 <- as.numeric(grep("350", colnames(Blank.interp)))
    
    # get the Raman correction file from the Raman function stored
    Raman.area <- Ramancor(blank = Blank.interp) 
    Raman[i,2] <- Raman.area
    Raman[i,1] <- samplename
    
    # normalize the EEM file for the area underneath the raman curve calculated above 
    # Raman normalize the raw EEM
    EEM.ram = EEM.interp/Raman.area 
    
    # Raman normalize the blank file (important for blank correction)
    blankram = Blank.interp/Raman.area
  ...
  # save file with Raman correction factors
  write.csv(Raman, paste0(directoryCorrectedEEMS, "/CRRamanfactors.csv"))
